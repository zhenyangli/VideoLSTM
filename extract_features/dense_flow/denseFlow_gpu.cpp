...
static void convertFlowToImage(const Mat &flow_x, const Mat &flow_y, Mat &img_x, Mat &img_y, Mat &img_flow,
	...
	for (int i = 0; i < flow_x.rows; ++i) {
		for (int j = 0; j < flow_y.cols; ++j) {
            ...
            float z = cvSqrt(x*x + y*y);
			img_x.at<uchar>(i,j) = CAST(x, lowerBound, higherBound);
			img_y.at<uchar>(i,j) = CAST(y, lowerBound, higherBound);

            img_flow.at<Vec3b>(i,j)[0] = CAST(x, lowerBound, higherBound);
            img_flow.at<Vec3b>(i,j)[1] = CAST(y, lowerBound, higherBound);
            img_flow.at<Vec3b>(i,j)[2] = CAST(z, lowerBound, higherBound);
		}
	}
	#undef CAST
}
...
int main(int argc, char** argv){
	...
	const char* keys =
		{
            ...
            "{ z  | flowFile     | flow   | filename of flow 3 component }"
			"{ i  | imgFile      | image  | filename of image}"
	...
	while(true) {
		...
		if(frame_num == 0) {
            ...
            resize(prev_image,prev_image_,cv::Size(340,256));
            imwrite(imgFile + tmp, prev_image_);
        ...
        Mat imgFlow(flow_x.size(),CV_8UC3);
		convertFlowToImage(flow_x, flow_y, imgX, imgY, imgFlow, -bound, bound);
		char tmp1[20];
        char tmp2[20];
		sprintf(tmp1,"_%04d.jpg",int(frame_num));
        sprintf(tmp2,"_%04d.jpg",int(frame_num+1));

		Mat imgX_, imgY_, imgFlow_, image_;
		resize(imgX,imgX_,cv::Size(340,256));
		resize(imgY,imgY_,cv::Size(340,256));
        resize(imgFlow,imgFlow_,cv::Size(340,256));
		resize(image,image_,cv::Size(340,256));

		imwrite(xFlowFile + tmp1, imgX_);
		imwrite(yFlowFile + tmp1, imgY_);
        imwrite(flowFile + tmp1, imgFlow_);
		imwrite(imgFile + tmp2, image_);
