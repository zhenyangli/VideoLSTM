...
class StackInterfaceLayer(object):
    def __init__(self, layer_param):
        ...
        context_data_type = layer_param.get('context_data_type', theano.config.floatX)
        context_ndim = layer_param.get('context_ndim', 5)
        ...
        self.context = quick_symbolic_variable(ndim=context_ndim, name=self._s("context"), typ=context_data_type)
    ...
    def set_name(self):
        self.name = "StackInterfaceLayer-" + str(self.id)
    ...
    def symbols(self):
        if self.use_mask:
            return self.input_symbols() + self.context_symbols() + self.mask_symbols() + self.output_symbols()
        else:
            return self.input_symbols() + self.context_symbols() + self.output_symbols()
    ...
    def print_stat(self):
        ...
        logger.info("   Context Type: " + str(self.input.type) + " Context Name: " + self.input.name)
