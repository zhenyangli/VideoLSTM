...
'''
class DeepCondLSTMDecpLayer(Layer):
    def __init__(self, layer_param):
        super(DeepCondLSTMDecpLayer, self).__init__(layer_param)
        ...
        self.Wc_att = quick_init_norm(self.rng, (self.feature_in, self.context_out), self._s("Wc_att"), scale=0.1)
        # attention: context -> hidden
        self.Wd_att = quick_init_norm(self.rng, (self.context_in, self.context_out), self._s("Wd_att"), scale=0.1)
        # attention: LSTM-pred -> hidden
        # self.We_att = quick_init_norm(self.rng, (self.feature_out, self.context_out), self._s("We_att"), scale=0.1)
        # attention: hidden bias
        self.b_att = quick_zero((self.context_out, ), self._s("b_att"))
        # optional "deep" attention
        # if options['n_layers_att'] > 1:
        #     for lidx in xrange(1, options['n_layers_att']):
        # attention:
        self.U_att = quick_init_norm(self.rng, (self.context_out, 1), self._s("U_att"), scale=0.1)
    ...
    def set_name(self):
        self.name = "DeepCondLSTMDecpLayer-" + str(self.id)

    def step_fprop(self, x_t, ctx_t, h_pred_tm1, c_pred_tm1, alpha_, attend_, *args):
        # x_t input @ t
        # ctx_t context @ t
        ...
        # attention mechanism
        pstate = TT.dot(ctx_t, self.Wd_att) # BS x DIN
        ...
        return [h_pred_t, c_pred_t, alpha, attend]

    def step_masked_fprop(self, x_t, ctx_t, mask_t, h_pred_tm1, c_pred_tm1, alpha_, attend_, *args):

        h_pred_t, c_pred_t, alpha, attend = self.step_fprop(x_t, ctx_t, \
                                               h_pred_tm1, c_pred_tm1, alpha_, attend_, *args)
    ...
    def fprop(self):
        ...
        non_seqs = self.param
        [self.output, self.cell_output, self.alpha, self.attend], self.output_update = \
