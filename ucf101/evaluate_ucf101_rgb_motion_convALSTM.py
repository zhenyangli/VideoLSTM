...
save_path = "./ucf101-experiment/ucf101-motion-convALSTM/rms-lr-0.001-drop-0.7/"
log_path = save_path + "evaluate_ucf101_motion_convALSTM.log"
...
############################# interface layer
param = {"id": "ucf101-vgg16-pool5", "use_mask": True,
         "input_ndim": 5, "context_ndim": 5, "output_ndim": 2,
...
context_dim = (ctx_dim, row_num, col_num)

logger.info("Data Dim:" + str(data_dim))
logger.info("Context Dim:" + str(context_dim))
...
ctx_mean = ctx.mean(0) ### ctx_mean is now (Minibatch, ContextDim, ROWS, COLS)
...
param = {"id": 2, "rng": rng, "theano_rng": theano_rng,
          "dim_in": context_dim, "dim_out": (hidden_dim, row_num, col_num),
          "receptive_field": (1, 1),
          "minibatch_size": minibatch_size,
          "activation": "tanh",
          "input": ctx_mean}
...
#4# conditional lstm layer (main layer)
param = {"id": 4, "rng": rng, "theano_rng": theano_rng,
         "dim_in": data_dim, "dim_out": (hidden_dim, row_num, col_num),
         "ctx_dim_in": context_dim, "ctx_dim_out": (hidden_dim, row_num, col_num),
         "input_receptive_field": (3, 3),
         "transition_receptive_field": (3, 3),
         "context_input_receptive_field": (1, 1),
         "context_transition_receptive_field": (1, 1),
         "minibatch_size": minibatch_size,
         ...
         "n_steps": seq_length}
middle_layers.append(DeepCondConvLSTMSoftLayer(param))
...
outputs = [{"name": "probability", "value": middle_layers[7].output}]
...
param = {'interface_layer': interface_layer, 'middle_layers': middle_layers, 'cost_layer': cost_layer,
         ...
         'name': "UCF101-VideoModel-Motion-convALSTM-RMS",
